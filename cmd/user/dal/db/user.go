...
type User struct {
	UserId        int64  `gorm:"column:user_id;type:bigint(20) unsigned" json:"user_id"`                      // 用户id，唯一
	...
	FollowCount   int    `gorm:"column:follow_count;type:int(10) unsigned;default:0" json:"follow_count"`     // 关注数，冗余字段
	FollowerCount int    `gorm:"column:follower_count;type:int(10) unsigned;default:0" json:"follower_count"` // 粉丝数，冗余字段
	gorm.Model
}
...
func QueryUser(ctx context.Context, username string) (*User, error) {
	user := &User{}
	if err := DB.WithContext(ctx).Where("username = ?", username).Take(&user).Error; err != nil {
		return nil, err
	}
	return user, nil
}
